...
export class LeaveTypesListComponent implements OnInit, OnDestroy {
    items: LeaveTypeDto[] = [];
    isLoading = false;
    totalCount = 0;
    currentPage = 0;
    ...
    displayedColumns = ['name', 'code', 'paid', 'maxDays', 'flags', 'status', 'actions'];
    Math = Math;
    ...
    load(): void {
        ...
        this._service.search({
            ...
            error: () => {
                this._notificationService.error('Failed to load leave types');
                this.isLoading = false;
                this._changeDetectorRef.markForCheck();
            },
        });
        ...
        confirmation.afterClosed().subscribe((result) => {
            if (result === 'confirmed') {
                this._service.delete(item.id).pipe(takeUntil(this._unsubscribeAll)).subscribe({
                    next: () => { this._notificationService.success('Leave type deleted'); this.load(); },
                    error: () => this._notificationService.error('Failed to delete leave type'),
                });
            }
        });
    }
