    ...
    <div class="relative z-10 flex flex-col flex-auto min-w-0">
        <div class="flex flex-col sm:flex-row space-y-16 sm:space-y-0 flex-0 sm:items-center sm:justify-between p-2 sm:py-3 sm:px-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h2 class="text-3xl font-bold tracking-tight leading-7 sm:leading-10 truncate bg-gradient-to-r from-gray-900 to-gray-600 dark:from-white dark:to-gray-300 bg-clip-text text-transparent">Branding Profiles</h2>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Receipt &amp; invoice templates — pick one as the default per outlet, or override per sale.</p>
                </div>
            </div>
            <div class="flex flex-col w-full sm:w-auto sm:flex-row space-y-16 sm:space-y-0 flex-1 sm:flex-none sm:items-center sm:justify-end gap-4">
                <mat-form-field class="w-full sm:w-auto sm:min-w-72" appearance="outline" subscriptSizing="dynamic">
                    ...
                    <input matInput [(ngModel)]="searchTerm" placeholder="Name / format / tax id">
                    <mat-icon matSuffix class="text-gray-400">search</mat-icon>
                </mat-form-field>
                <button mat-fab color="primary" routerLink="create" matTooltip="Add new branding profile">
                ...
                <div class="relative overflow-x-auto">
                    <table mat-table [dataSource]="filtered()" class="w-full">
                        <ng-container matColumnDef="name">
                            <th mat-header-cell *matHeaderCellDef class="pl-4 sm:pl-6"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">Profile</span></th>
                        ...
                        <ng-container matColumnDef="format">
                            <th mat-header-cell *matHeaderCellDef><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">Format</span></th>
                        ...
                        <ng-container matColumnDef="color">
                            <th mat-header-cell *matHeaderCellDef><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">Brand</span></th>
                            <td mat-cell *matCellDef="let r">
                                @if (r.primaryColor) {
                        ...
                        <ng-container matColumnDef="actions">
                            ...
                            <td mat-cell *matCellDef="let r" class="pr-4 sm:pr-6">
                                <div class="flex items-center justify-end space-x-2">
                                    ...
                                    <button mat-icon-button class="text-red-600" (click)="remove(r)" matTooltip="Delete (blocked if any sale uses it)"><mat-icon class="icon-size-5">delete</mat-icon></button>
                                </div>
                            </td>
                        </ng-container>
                ...
                @if (!loading() && filtered().length === 0) {
                    <div class="flex flex-col items-center justify-center p-12">
                        ...
                        <h3 class="text-xl font-semibold text-gray-900 dark:text-white mb-3">
                            {{ rows().length === 0 ? 'No branding profiles yet' : 'No matches' }}
                        </h3>
                        <p class="text-sm text-gray-500 mb-4 max-w-md text-center">
                            Each profile bundles a logo, color, tax id and paper format. Outlets pick a default; cashiers can override per sale.
                        </p>
                        @if (rows().length === 0) {
                            <button mat-flat-button color="primary" routerLink="create"><mat-icon class="icon-size-5 mr-2">add_circle</mat-icon><span>Add Profile</span></button>
                        }
                    </div>
                }
            </div>
