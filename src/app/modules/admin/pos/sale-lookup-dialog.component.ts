...
@Component({
    ...
    imports: [
        ...
        MatFormFieldModule, MatIconModule, MatInputModule, MatTableModule, MatTooltipModule, TranslocoModule,
    ],
    template: `
<h2 mat-dialog-title class="!flex !items-center !gap-2">
    <mat-icon class="text-blue-600">receipt_long</mat-icon>
    <span>{{ 'POS.SALE_LOOKUP.TITLE' | transloco }}</span>
</h2>
<mat-dialog-content class="!min-w-[640px] !max-w-[800px]">
    <div class="flex items-center gap-3 mb-3">
        <mat-form-field class="flex-1" appearance="outline" subscriptSizing="dynamic">
            <mat-label>{{ 'POS.SALE_LOOKUP.SEARCH_LABEL' | transloco }}</mat-label>
            <input matInput [(ngModel)]="search" (ngModelChange)="searchChanged.next($event)"
                   [placeholder]="'POS.SALE_LOOKUP.SEARCH_PLACEHOLDER' | transloco" autofocus>
            <mat-icon matSuffix class="text-gray-400">search</mat-icon>
        </mat-form-field>
    </div>
    <div class="text-xs text-gray-500 mb-2" *ngIf="data.outletName">
        {{ 'POS.SALE_LOOKUP.SHOWING_FOR_OUTLET_PREFIX' | transloco }} <span class="font-semibold">{{ data.outletName }}</span>{{ 'POS.SALE_LOOKUP.SHOWING_FOR_OUTLET_SUFFIX' | transloco }}
    </div>

    <div class="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
        <table mat-table [dataSource]="rows()" class="w-full">
            <ng-container matColumnDef="invoice"><th mat-header-cell *matHeaderCellDef class="pl-4"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'POS.SALE_LOOKUP.COL_INVOICE' | transloco }}</span></th>
                <td mat-cell *matCellDef="let r" class="pl-4 font-mono text-sm">{{ r.invoiceNumber }}</td></ng-container>
            <ng-container matColumnDef="date"><th mat-header-cell *matHeaderCellDef><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'POS.SALE_LOOKUP.COL_DATE' | transloco }}</span></th>
                <td mat-cell *matCellDef="let r" class="text-xs">{{ r.saleDate | date:'short' }}</td></ng-container>
            <ng-container matColumnDef="customer"><th mat-header-cell *matHeaderCellDef><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'POS.SALE_LOOKUP.COL_CUSTOMER' | transloco }}</span></th>
                <td mat-cell *matCellDef="let r">
                    <div class="flex flex-col">
                        <span class="text-sm">{{ r.customerName || ('POS.SALE_LOOKUP.WALK_IN' | transloco) }}</span>
                        <span class="text-xs text-gray-500">{{ r.customerPhone || '—' }}</span>
                    </div>
                </td></ng-container>
            <ng-container matColumnDef="total"><th mat-header-cell *matHeaderCellDef class="!text-right"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'POS.SALE_LOOKUP.COL_TOTAL' | transloco }}</span></th>
            ...
            <ng-container matColumnDef="actions"><th mat-header-cell *matHeaderCellDef class="pr-4 !text-right"></th>
                <td mat-cell *matCellDef="let r" class="pr-4">
                    <div class="flex items-center justify-end gap-1">
                        <button mat-icon-button class="text-blue-600" (click)="reprint(r)" [matTooltip]="'POS.SALE_LOOKUP.REPRINT_TOOLTIP' | transloco"><mat-icon class="icon-size-5">print</mat-icon></button>
                        <button mat-icon-button class="text-gray-600" (click)="open(r)" [matTooltip]="'POS.SALE_LOOKUP.OPEN_TOOLTIP' | transloco"><mat-icon class="icon-size-5">open_in_new</mat-icon></button>
                    </div>
                </td></ng-container>
        ...
        <div *ngIf="!loading() && rows().length === 0" class="p-8 text-center text-gray-500">
            ...
            <div class="text-sm">{{ 'POS.SALE_LOOKUP.EMPTY' | transloco }}</div>
        </div>
        <div *ngIf="loading()" class="p-8 text-center text-gray-400 text-sm">{{ 'POS.SALE_LOOKUP.LOADING' | transloco }}</div>
    </div>
</mat-dialog-content>
