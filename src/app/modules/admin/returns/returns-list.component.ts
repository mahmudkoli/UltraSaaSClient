    ...
    <div class="relative z-10 flex flex-col flex-auto min-w-0">
        <div class="flex flex-col sm:flex-row space-y-16 sm:space-y-0 flex-0 sm:items-center sm:justify-between p-2 sm:py-3 sm:px-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h2 class="text-3xl font-bold tracking-tight leading-7 sm:leading-10 truncate bg-gradient-to-r from-gray-900 to-gray-600 dark:from-white dark:to-gray-300 bg-clip-text text-transparent">{{ 'RETURNS.LIST.TITLE' | transloco }}</h2>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ 'RETURNS.LIST.SUBTITLE' | transloco }}</p>
                </div>
            </div>
            <div class="flex flex-col w-full sm:w-auto sm:flex-row space-y-16 sm:space-y-0 flex-1 sm:flex-none sm:items-center sm:justify-end gap-4">
                <mat-form-field class="w-full sm:w-auto sm:min-w-72" appearance="outline" subscriptSizing="dynamic">
                    <mat-label>{{ 'RETURNS.LIST.SEARCH_LABEL' | transloco }}</mat-label>
                    <input matInput [(ngModel)]="search" (ngModelChange)="searchChanged.next($event)" [placeholder]="'RETURNS.LIST.SEARCH_PLACEHOLDER' | transloco">
                    <mat-icon matSuffix class="text-gray-400">search</mat-icon>
                </mat-form-field>
                <mat-form-field class="w-full sm:w-auto sm:min-w-48" appearance="outline" subscriptSizing="dynamic">
                    <mat-label>{{ 'RETURNS.LIST.OUTLET_LABEL' | transloco }}</mat-label>
                    <mat-select [(ngModel)]="outletFilter" (ngModelChange)="resetAndLoad()">
                        <mat-option [value]="''">{{ 'RETURNS.LIST.OUTLET_ALL' | transloco }}</mat-option>
                ...
                <mat-form-field class="w-full sm:w-auto sm:min-w-44" appearance="outline" subscriptSizing="dynamic">
                    <mat-label>{{ 'RETURNS.LIST.STATUS_LABEL' | transloco }}</mat-label>
                    <mat-select [(ngModel)]="statusFilter" (ngModelChange)="resetAndLoad()">
                        <mat-option value="all">{{ 'RETURNS.LIST.STATUS_ALL' | transloco }}</mat-option>
                        <mat-option value="Completed">{{ 'RETURNS.LIST.STATUS_COMPLETED' | transloco }}</mat-option>
                        <mat-option value="Voided">{{ 'RETURNS.LIST.STATUS_VOIDED' | transloco }}</mat-option>
                        <mat-option value="Draft">{{ 'RETURNS.LIST.STATUS_DRAFT' | transloco }}</mat-option>
                    </mat-select>
                </mat-form-field>
                <button mat-fab color="primary" routerLink="/sales" [matTooltip]="'RETURNS.LIST.FIND_SALE_TOOLTIP' | transloco"><mat-icon>search</mat-icon></button>
            </div>
        </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div class="relative overflow-x-auto">
                    <table mat-table matSort [dataSource]="rows()" (matSortChange)="onSort($event)" class="w-full">
                        <ng-container matColumnDef="returnNumber"><th mat-header-cell *matHeaderCellDef mat-sort-header class="pl-4 sm:pl-6"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'RETURNS.LIST.COL_RETURN_NUMBER' | transloco }}</span></th>
                            <td mat-cell *matCellDef="let r" class="pl-4 sm:pl-6 font-mono text-sm">{{ r.returnNumber }}</td></ng-container>
                        <ng-container matColumnDef="invoice"><th mat-header-cell *matHeaderCellDef><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'RETURNS.LIST.COL_INVOICE' | transloco }}</span></th>
                        ...
                        <ng-container matColumnDef="returnDate"><th mat-header-cell *matHeaderCellDef mat-sort-header><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'RETURNS.LIST.COL_DATE' | transloco }}</span></th>
                            <td mat-cell *matCellDef="let r">{{ r.returnDate | date:'short' }}</td></ng-container>
                        <ng-container matColumnDef="customer"><th mat-header-cell *matHeaderCellDef><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'RETURNS.LIST.COL_CUSTOMER' | transloco }}</span></th>
                            <td mat-cell *matCellDef="let r">{{ r.customerName || ('RETURNS.LIST.WALK_IN' | transloco) }}</td></ng-container>
                        <ng-container matColumnDef="reason"><th mat-header-cell *matHeaderCellDef><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'RETURNS.LIST.COL_REASON' | transloco }}</span></th>
                            <td mat-cell *matCellDef="let r">{{ r.reason }}</td></ng-container>
                        <ng-container matColumnDef="items"><th mat-header-cell *matHeaderCellDef class="!text-right"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'RETURNS.LIST.COL_ITEMS' | transloco }}</span></th>
                            <td mat-cell *matCellDef="let r" class="!text-right">{{ r.items.length }}</td></ng-container>
                        <ng-container matColumnDef="refundAmount"><th mat-header-cell *matHeaderCellDef mat-sort-header class="!text-right"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'RETURNS.LIST.COL_REFUND' | transloco }}</span></th>
                        ...
                        <ng-container matColumnDef="status"><th mat-header-cell *matHeaderCellDef mat-sort-header><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'RETURNS.LIST.COL_STATUS' | transloco }}</span></th>
                        ...
                        <ng-container matColumnDef="actions"><th mat-header-cell *matHeaderCellDef class="pr-4 sm:pr-6 !text-right"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'RETURNS.LIST.COL_ACTIONS' | transloco }}</span></th>
                            <td mat-cell *matCellDef="let r" class="pr-4 sm:pr-6">
                                <div class="flex items-center justify-end space-x-2">
                                    <button mat-icon-button class="text-blue-600" (click)="$event.stopPropagation(); view(r)" [matTooltip]="'RETURNS.LIST.VIEW_TOOLTIP' | transloco"><mat-icon class="icon-size-5">visibility</mat-icon></button>
                                </div>
                            </td></ng-container>
                ...
                <div *ngIf="!loading() && rows().length === 0" class="flex flex-col items-center justify-center p-12">
                    ...
                    <h3 class="text-xl font-semibold text-gray-900 dark:text-white mb-3">{{ 'RETURNS.LIST.EMPTY_TITLE' | transloco }}</h3>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mb-6">{{ 'RETURNS.LIST.EMPTY_SUBTITLE' | transloco }}</p>
                    <button mat-flat-button color="primary" routerLink="/sales"><mat-icon class="icon-size-5 mr-2">receipt_long</mat-icon><span>{{ 'RETURNS.LIST.BROWSE_SALES' | transloco }}</span></button>
                </div>
            </div>
        </div>
    </div>
...
export class ReturnsListComponent implements OnInit {
    ...
    statusLabel(s: string): string {
        if (s === 'Completed') return 'RETURNS.LIST.STATUS_COMPLETED';
        if (s === 'Voided') return 'RETURNS.LIST.STATUS_VOIDED';
        return 'RETURNS.LIST.STATUS_DRAFT';
    }
