...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, RouterModule, TranslocoModule, MatButtonModule, MatDialogModule, MatIconModule, MatMenuModule, MatTableModule, MatTooltipModule],
    template: `
            ...
            <div class="flex flex-col sm:flex-row space-y-16 sm:space-y-0 flex-0 sm:items-center sm:justify-between p-2 sm:py-3 sm:px-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ s.saleDate | date:'medium' }} · {{ s.customerName || ('SALES.DETAIL.WALK_IN' | transloco) }}</p>
                    </div>
                </div>
                <div class="flex flex-col w-full sm:w-auto sm:flex-row space-y-16 sm:space-y-0 flex-1 sm:flex-none sm:items-center sm:justify-end gap-4">
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium"
                        ...
                        <mat-icon class="icon-size-4 mr-1">{{ s.status === 'Finalized' ? 'check_circle' : s.status === 'Voided' ? 'cancel' : 'schedule' }}</mat-icon>{{ saleStatusLabel(s.status) | transloco }}
                    </span>
                    @if (s.status === 'Finalized') {
                        <button mat-stroked-button class="h-12 px-6 rounded-lg" (click)="print(s)"><mat-icon class="icon-size-5 mr-2">print</mat-icon><span>{{ 'SALES.DETAIL.PRINT_RECEIPT' | transloco }}</span></button>
                        <button mat-flat-button class="h-12 px-6 rounded-lg !bg-emerald-600 !text-white" (click)="share(s)" [matTooltip]="'SALES.DETAIL.SHARE_TOOLTIP' | transloco"><mat-icon class="icon-size-5 mr-2">share</mat-icon><span>{{ 'SALES.DETAIL.SHARE' | transloco }}</span></button>
                        @if (profiles().length > 0) {
                            <button mat-icon-button class="h-12 w-12 rounded-lg" [matMenuTriggerFor]="printMenu" [matTooltip]="'SALES.DETAIL.REPRINT_AS_TOOLTIP' | transloco"><mat-icon>more_vert</mat-icon></button>
                            <mat-menu #printMenu="matMenu">
                                <button mat-menu-item (click)="print(s)"><mat-icon>print</mat-icon><span>{{ 'SALES.DETAIL.AS_SAVED' | transloco }}</span></button>
                                <div class="text-xs text-gray-500 px-4 py-1 uppercase tracking-wider">{{ 'SALES.DETAIL.OVERRIDE_PROFILE' | transloco }}</div>
                    ...
                    <button mat-stroked-button class="h-12 px-6 rounded-lg" routerLink="/sales"><mat-icon class="icon-size-5 mr-2">arrow_back</mat-icon><span>{{ 'SALES.DETAIL.BACK' | transloco }}</span></button>
                </div>
            </div>
                    ...
                    <div class="lg:col-span-2 flex flex-col gap-6">
                        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center space-x-3">
                                ...
                                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ 'SALES.DETAIL.ITEMS' | transloco }}</h3>
                            </div>
                            <table mat-table [dataSource]="s.items" class="w-full">
                                <ng-container matColumnDef="sku"><th mat-header-cell *matHeaderCellDef class="pl-6"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'SALES.DETAIL.COL_SKU' | transloco }}</span></th>
                                    <td mat-cell *matCellDef="let i" class="pl-6 font-mono text-xs">{{ i.sku }}</td></ng-container>
                                <ng-container matColumnDef="name"><th mat-header-cell *matHeaderCellDef><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'SALES.DETAIL.COL_PRODUCT' | transloco }}</span></th>
                                    <td mat-cell *matCellDef="let i">
                                        <div class="flex flex-col">
                                            <span class="text-sm font-medium text-gray-900 dark:text-white">{{ i.productName }}</span>
                                            <span class="text-xs text-gray-500" *ngIf="i.serialNumber">{{ 'SALES.DETAIL.SN_PREFIX' | transloco }}: {{ i.serialNumber }}</span>
                                            <span class="text-xs text-gray-500" *ngIf="i.batchNumber">{{ 'SALES.DETAIL.BATCH_PREFIX' | transloco }}: {{ i.batchNumber }}</span>
                                        </div>
                                    </td></ng-container>
                                <ng-container matColumnDef="qty"><th mat-header-cell *matHeaderCellDef class="!text-right"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'SALES.DETAIL.COL_QTY' | transloco }}</span></th>
                                    <td mat-cell *matCellDef="let i" class="!text-right">{{ i.quantity | number:'1.0-3' }}</td></ng-container>
                                <ng-container matColumnDef="unit"><th mat-header-cell *matHeaderCellDef class="!text-right"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'SALES.DETAIL.COL_UNIT' | transloco }}</span></th>
                                    <td mat-cell *matCellDef="let i" class="!text-right">{{ i.unitPrice | number:'1.2-2' }}</td></ng-container>
                                <ng-container matColumnDef="discount"><th mat-header-cell *matHeaderCellDef class="!text-right"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'SALES.DETAIL.COL_DISCOUNT' | transloco }}</span></th>
                                    <td mat-cell *matCellDef="let i" class="!text-right">{{ i.discountAmount | number:'1.2-2' }}</td></ng-container>
                                <ng-container matColumnDef="total"><th mat-header-cell *matHeaderCellDef class="pr-6 !text-right"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'SALES.DETAIL.COL_TOTAL' | transloco }}</span></th>
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center space-x-3">
                                ...
                                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ 'SALES.DETAIL.PAYMENTS' | transloco }}</h3>
                            </div>
                            <table mat-table [dataSource]="s.payments" class="w-full">
                                <ng-container matColumnDef="method"><th mat-header-cell *matHeaderCellDef class="pl-6"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'SALES.DETAIL.COL_METHOD' | transloco }}</span></th>
                                    <td mat-cell *matCellDef="let p" class="pl-6">{{ p.method }}</td></ng-container>
                                <ng-container matColumnDef="amount"><th mat-header-cell *matHeaderCellDef class="!text-right"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'SALES.DETAIL.COL_AMOUNT' | transloco }}</span></th>
                                    <td mat-cell *matCellDef="let p" class="!text-right font-medium">{{ p.amount | number:'1.2-2' }}</td></ng-container>
                                <ng-container matColumnDef="ref"><th mat-header-cell *matHeaderCellDef><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'SALES.DETAIL.COL_REFERENCE' | transloco }}</span></th>
                                    <td mat-cell *matCellDef="let p">{{ p.reference || '—' }}</td></ng-container>
                                <ng-container matColumnDef="paid"><th mat-header-cell *matHeaderCellDef class="pr-6"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'SALES.DETAIL.COL_PAID_ON' | transloco }}</span></th>
                    ...
                    <div class="flex flex-col gap-6">
                        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden p-6">
                            <div class="flex items-center space-x-3 mb-4">
                                ...
                                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ 'SALES.DETAIL.SUMMARY' | transloco }}</h3>
                            </div>
                            <div class="space-y-2 text-sm">
                                <div class="flex justify-between"><span class="text-gray-600 dark:text-gray-400">{{ 'SALES.DETAIL.SUBTOTAL' | transloco }}</span><span class="font-medium">{{ s.subTotal | number:'1.2-2' }}</span></div>
                                <div class="flex justify-between"><span class="text-gray-600 dark:text-gray-400">{{ 'SALES.DETAIL.DISCOUNT' | transloco }}</span><span class="font-medium text-red-600">−{{ s.discountAmount | number:'1.2-2' }}</span></div>
                                <div class="flex justify-between"><span class="text-gray-600 dark:text-gray-400">{{ 'SALES.DETAIL.TAX' | transloco }}</span><span class="font-medium">{{ s.taxAmount | number:'1.2-2' }}</span></div>
                                <div class="flex justify-between text-xl font-bold pt-2 border-t border-gray-200 dark:border-gray-700"><span>{{ 'SALES.DETAIL.TOTAL' | transloco }}</span><span>{{ s.total | number:'1.2-2' }}</span></div>
                                <div class="flex justify-between text-sm pt-2"><span class="text-gray-600 dark:text-gray-400">{{ 'SALES.DETAIL.PAID' | transloco }}</span><span class="text-green-600 font-medium">{{ s.paidAmount | number:'1.2-2' }}</span></div>
                                <div class="flex justify-between text-sm"><span class="text-gray-600 dark:text-gray-400">{{ 'SALES.DETAIL.BALANCE' | transloco }}</span><span class="font-medium" [class.text-red-600]="s.balance > 0">{{ s.balance | number:'1.2-2' }}</span></div>
                                @if (resolvedProfile(); as rp) {
                                    <div class="flex justify-between text-xs pt-2 border-t border-gray-100 dark:border-gray-700 mt-1">
                                        <span class="text-gray-500">{{ 'SALES.DETAIL.RECEIPT_TEMPLATE' | transloco }}</span>
                        ...
                        @if (s.status === 'Finalized') {
                            <button mat-stroked-button color="warn" class="h-12 rounded-lg" [routerLink]="['/returns/new', s.id]"
                                    ...
                                    [matTooltip]="(fullyReturned() ? 'SALES.DETAIL.FULLY_RETURNED_TOOLTIP' : 'SALES.DETAIL.PROCESS_RETURN_TOOLTIP') | transloco">
                                <mat-icon class="icon-size-5 mr-2">undo</mat-icon>
                                <span>{{ (fullyReturned() ? 'SALES.DETAIL.FULLY_RETURNED' : 'SALES.DETAIL.PROCESS_RETURN') | transloco }}</span>
                            </button>
                        }
                    </div>
                </div>
                ...
                @if (returns().length > 0) {
                    <div class="mt-6 bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center space-x-3">
                            ...
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ 'SALES.DETAIL.RETURNS' | transloco }}</h3>
                            <span class="ml-2 inline-flex items-center gap-1 text-xs font-medium text-rose-700 dark:text-rose-300 bg-rose-50 dark:bg-rose-900/30 rounded-full px-2 py-0.5">
                                {{ 'SALES.DETAIL.RETURNS_BADGE' | transloco:{ count: returns().length, amount: (totalRefunded() | number:'1.2-2') } }}
                            </span>
                        </div>
                        <table mat-table [dataSource]="returns()" class="w-full">
                            <ng-container matColumnDef="returnNumber"><th mat-header-cell *matHeaderCellDef class="pl-6"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'SALES.DETAIL.COL_RETURN_NUMBER' | transloco }}</span></th>
                            ...
                            <ng-container matColumnDef="date"><th mat-header-cell *matHeaderCellDef><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'SALES.DETAIL.COL_DATE' | transloco }}</span></th>
                                <td mat-cell *matCellDef="let r">{{ r.returnDate | date:'short' }}</td></ng-container>
                            <ng-container matColumnDef="reason"><th mat-header-cell *matHeaderCellDef><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'SALES.DETAIL.COL_REASON' | transloco }}</span></th>
                                <td mat-cell *matCellDef="let r">{{ r.reason }}</td></ng-container>
                            <ng-container matColumnDef="items"><th mat-header-cell *matHeaderCellDef class="!text-right"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'SALES.DETAIL.ITEMS' | transloco }}</span></th>
                                <td mat-cell *matCellDef="let r" class="!text-right tabular-nums">{{ returnedItemCount(r) }}</td></ng-container>
                            <ng-container matColumnDef="refund"><th mat-header-cell *matHeaderCellDef class="!text-right"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'SALES.DETAIL.COL_REFUND' | transloco }}</span></th>
                            ...
                            <ng-container matColumnDef="status"><th mat-header-cell *matHeaderCellDef class="pr-6"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'SALES.DETAIL.COL_STATUS' | transloco }}</span></th>
                                <td mat-cell *matCellDef="let r" class="pr-6">
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium"
                                          ...
                                          }">{{ returnStatusLabel(r.status) | transloco }}</span>
                                </td></ng-container>
...
export class SaleDetailComponent implements OnInit {
    ...
    /** Translation-key for the sale's status pill. */
    saleStatusLabel(status: string): string {
        if (status === 'Finalized') return 'SALES.DETAIL.STATUS_FINALIZED';
        if (status === 'Voided') return 'SALES.DETAIL.STATUS_VOIDED';
        return 'SALES.DETAIL.STATUS_DRAFT';
    }

    /** Translation-key for a return row's status pill. */
    returnStatusLabel(status: string): string {
        if (status === 'Completed') return 'SALES.DETAIL.RETURN_STATUS_COMPLETED';
        if (status === 'Voided') return 'SALES.DETAIL.RETURN_STATUS_VOIDED';
        return 'SALES.DETAIL.RETURN_STATUS_DRAFT';
    }
