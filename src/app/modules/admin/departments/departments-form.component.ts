...
export class DepartmentsFormComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        if (this.isEditMode && this.id) {
            ...
            this._service.getById(this.id).pipe(takeUntil(this._unsubscribeAll)).subscribe({
                ...
                error: () => {
                    this._notificationService.error('Error loading department');
                    this.isLoading = false;
                    this._changeDetectorRef.markForCheck();
                },
            });
    ...
    save(): void {
        ...
        const done = {
            next: () => {
                this.isSaving = false;
                this._notificationService.success(`Department ${this.isEditMode ? 'updated' : 'created'}`);
                this._router.navigate(['/departments']);
            },
            error: () => {
                this.isSaving = false;
                this._changeDetectorRef.markForCheck();
                this._notificationService.error(`Error ${this.isEditMode ? 'updating' : 'creating'} department`);
            },
        };
        if (this.isEditMode && this.id) {
