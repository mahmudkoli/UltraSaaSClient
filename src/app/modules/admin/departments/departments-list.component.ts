...
export class DepartmentsListComponent implements OnInit, OnDestroy {
    ...
    load(): void {
        ...
        this._service.search({
            ...
            error: () => {
                this._notificationService.error('Failed to load departments');
                this.isLoading = false;
                this._changeDetectorRef.markForCheck();
            },
        });
        ...
        confirmation.afterClosed().subscribe((result) => {
            if (result === 'confirmed') {
                this._service.delete(item.id).pipe(takeUntil(this._unsubscribeAll)).subscribe({
                    next: () => { this._notificationService.success('Department deleted'); this.load(); },
                    error: () => this._notificationService.error('Failed to delete department'),
                });
            }
        });
    }
