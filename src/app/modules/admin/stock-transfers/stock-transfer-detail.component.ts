                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                            ...
                            · created {{ t.createdOnUtc | date:'medium' }}
                        </p>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium" [ngClass]="statusClass(t.status)">
                        <mat-icon class="icon-size-4 mr-1">{{ statusIcon(t.status) }}</mat-icon>{{ t.status }}
                    </span>
                    <button mat-stroked-button class="h-12 px-6 rounded-lg" routerLink="/stock-transfers"><mat-icon class="icon-size-5 mr-2">arrow_back</mat-icon><span>Back</span></button>
                </div>
            </div>
            ...
            <div class="flex-auto p-4 sm:p-6">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold">Items</h3></div>
                        <table mat-table [dataSource]="t.items" class="w-full">
                            ...
                            <ng-container matColumnDef="qty"><th mat-header-cell *matHeaderCellDef class="pr-6 !text-right"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">Quantity</span></th>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden p-6 flex flex-col gap-3">
                        ...
                        @if (t.notes) {
                            <p class="text-xs text-gray-500">Notes: {{ t.notes }}</p>
                        }
                        @if (t.dispatchedOnUtc) { <p class="text-xs text-gray-500">Dispatched: {{ t.dispatchedOnUtc | date:'medium' }}</p> }
                        @if (t.receivedOnUtc) { <p class="text-xs text-gray-500">Received: {{ t.receivedOnUtc | date:'medium' }}</p> }

                        @if (t.status === 'Draft') {
                            <button mat-flat-button color="primary" class="w-full h-12 rounded-lg" [disabled]="busy()" (click)="dispatch()">
                                <mat-icon class="icon-size-5 mr-2">local_shipping</mat-icon><span>Dispatch (decrement source)</span>
                            </button>
                        ...
                        @if (t.status === 'InTransit') {
                            <button mat-flat-button color="primary" class="w-full h-12 rounded-lg" [disabled]="busy()" (click)="receive()">
                                <mat-icon class="icon-size-5 mr-2">inbox_arrow_down</mat-icon><span>Receive (increment destination)</span>
                            </button>
                        }
