...
import { HostelDto, HostelsService } from '../../../core/hostel/hostel.service';
import { NotificationService } from '../../../core/services/notification.service';

@Component({
    selector: 'hostels-list',
    templateUrl: './hostels-list.component.html',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    standalone: true,
    imports: [CommonModule, ReactiveFormsModule, RouterModule, MatButtonModule, MatFormFieldModule, MatIconModule, MatInputModule, MatPaginatorModule, MatProgressBarModule, MatTableModule, MatTooltipModule],
})
export class HostelsListComponent implements OnInit, OnDestroy {
    rows: HostelDto[] = [];
    total = 0;
    pageIndex = 0;
    pageSize = 25;
    ...
    load(): void {
        ...
        this._svc.search({ pageNumber: this.pageIndex + 1, pageSize: this.pageSize, keyword: this.search.value || undefined })
            .pipe(takeUntil(this._destroyed$)).subscribe({
                ...
                error: () => { this.loading = false; this._cdr.markForCheck(); this._notify.error('Could not load hostels.'); },
            });
    }

    onPage(e: PageEvent): void { this.pageIndex = e.pageIndex; this.pageSize = e.pageSize; this.load(); }
    add(): void { this._router.navigate(['/hostels/create']); }
    edit(h: HostelDto): void { this._router.navigate(['/hostels', h.id, 'edit']); }
    remove(h: HostelDto): void {
        if (!confirm(`Delete hostel "${h.name}"?`)) return;
        this._svc.delete(h.id).pipe(takeUntil(this._destroyed$)).subscribe({
