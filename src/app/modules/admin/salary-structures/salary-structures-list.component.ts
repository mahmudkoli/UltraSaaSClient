...
@Component({
    selector: 'salary-structures-list',
    templateUrl: './salary-structures-list.component.html',
...
export class SalaryStructuresListComponent implements OnInit, OnDestroy {
    ...
    displayedColumns = ['name', 'basic', 'components', 'status', 'actions'];
    Math = Math;
    ...
    load(): void {
        ...
        this._service.search({
            ...
            error: () => {
                this._notificationService.error('Failed to load salary structures');
                this.isLoading = false;
                this._changeDetectorRef.markForCheck();
            },
        });
    ...
    edit(item: SalaryStructureDto): void {
        this._router.navigate(['edit', item.id], { relativeTo: this._activatedRoute });
    }

    remove(item: SalaryStructureDto): void {
        ...
        confirmation.afterClosed().subscribe((result) => {
            if (result === 'confirmed') {
                this._service.delete(item.id).pipe(takeUntil(this._unsubscribeAll)).subscribe({
                    next: () => { this._notificationService.success('Salary structure deleted'); this.load(); },
                    error: () => this._notificationService.error('Failed to delete salary structure'),
                });
            }
        });
    }
