    ...
    <div class="p-4 sm:p-6">
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500">Today's pulse — sales, stock, shifts.</p>
            </div>
        </div>
        ...
        @if (dash(); as d) {
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4 mb-6">
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-4 border border-gray-200 dark:border-gray-700 col-span-2">
                    <div class="text-xs uppercase tracking-wider text-gray-500">Today's revenue (BDT)</div>
                    ...
                    <div class="text-xs mt-1">
                        @if (d.todayRevenueDeltaPct != null) {
                            <span [class.text-emerald-600]="d.todayRevenueDeltaPct >= 0" [class.text-rose-600]="d.todayRevenueDeltaPct < 0">
                                {{ d.todayRevenueDeltaPct >= 0 ? '+' : '' }}{{ d.todayRevenueDeltaPct }}% vs yesterday
                            </span>
                        } @else {
                            <span class="text-gray-400">No sales yesterday to compare</span>
                        }
                    </div>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-4 border border-gray-200 dark:border-gray-700">
                    <div class="text-xs uppercase tracking-wider text-gray-500">Sales today</div>
                    <div class="text-2xl font-bold mt-1">{{ d.todaySaleCount }}</div>
                    <div class="text-xs text-gray-400 mt-1">finalized invoices</div>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-4 border border-gray-200 dark:border-gray-700">
                    <div class="text-xs uppercase tracking-wider text-gray-500">Top product</div>
                    @if (d.topProductToday; as t) {
                        <div class="text-sm font-semibold mt-1 line-clamp-1" [matTooltip]="t.productName">{{ t.productName }}</div>
                        <div class="text-xs text-gray-500 mt-1">{{ t.quantitySold | number:'1.0-3' }} sold</div>
                    } @else {
                        <div class="text-sm text-gray-400 mt-1">No sales yet</div>
                    }
                </div>
                <a [routerLink]="['/inventory']" class="block bg-white dark:bg-gray-800 rounded-xl shadow-md p-4 border border-gray-200 dark:border-gray-700 hover:shadow-lg transition no-underline">
                    <div class="text-xs uppercase tracking-wider text-gray-500">Low stock</div>
                    <div class="text-2xl font-bold mt-1" [class.text-amber-600]="d.lowStockCount > 0">{{ d.lowStockCount }}</div>
                    <div class="text-xs text-gray-400 mt-1">item(s) below reorder</div>
                </a>
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-4 border border-gray-200 dark:border-gray-700">
                    <div class="text-xs uppercase tracking-wider text-gray-500">Open shift</div>
                    @if (d.openShift; as s) {
                        <a [routerLink]="['/shifts', s.shiftId, 'report']" class="block">
                            <div class="text-sm font-semibold mt-1 text-emerald-600">Open</div>
                            <div class="text-xs text-gray-500 mt-1">since {{ s.openedAt | date:'shortTime' }}</div>
                        </a>
                    } @else {
                        <div class="text-sm text-amber-600 font-semibold mt-1">None open</div>
                        <a routerLink="/shifts" class="text-xs underline">Open one →</a>
                    }
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-2xl shadow border border-gray-200 dark:border-gray-700">
                    ...
                    @if (d.recentSales.length === 0) {
                        <div class="p-6 text-center text-gray-500 text-sm">No finalized sales yet.</div>
                    } @else {
                            ...
                            @for (s of d.recentSales; track s.id) {
                                <li class="px-5 py-3 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                    <div>
                                        ...
                                        <div class="text-xs text-gray-500">{{ s.customerName || 'Walk-in' }} · {{ s.saleDate | date:'shortTime' }}</div>
                                    </div>
                                    <div class="text-lg font-semibold">BDT {{ s.total | number:'1.0-2' }}</div>
                                </li>
                            }
                        </ul>
                    }
                ...
                @if (d.outlets.length > 0) {
                    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow border border-gray-200 dark:border-gray-700">
                        <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center gap-3">
                            ...
                            <h3 class="text-lg font-semibold">By outlet (today)</h3>
                        </div>
                        <ul class="divide-y divide-gray-200 dark:divide-gray-700">
                            @for (o of d.outlets; track o.outletId) {
                                <li class="px-5 py-3 flex items-center justify-between">
                                    <div>
                                        ...
                                        <div class="text-xs text-gray-500">{{ o.todaySaleCount }} sale(s)</div>
                                    </div>
                                    <div class="text-sm font-semibold">BDT {{ o.todayRevenue | number:'1.0-2' }}</div>
                                </li>
                            }
                        </ul>
                    </div>
                    ...
                    <ul class="divide-y divide-gray-200 dark:divide-gray-700">
                        @for (a of d.actionItems; track a.kind) {
                            <li class="px-5 py-3 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                ...
                                <a [routerLink]="a.linkUrl" class="text-sm text-indigo-600 hover:underline">View →</a>
                            </li>
                        }
                    </ul>
                </div>
            ...
            <div class="flex items-center gap-3 text-gray-500"><mat-icon class="icon-size-5 animate-spin">progress_activity</mat-icon><span>Loading dashboard…</span></div>
        } @else if (errorMsg()) {
