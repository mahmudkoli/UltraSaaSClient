...
<div class="flex flex-col flex-auto min-w-0 p-4 sm:p-6">
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500">Plan, renewal status, and payment history for this shop.</p>
        </div>
    </div>
    ...
    @if (sub(); as s) {
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow border border-gray-200 dark:border-gray-700 p-5 mb-6">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div>
                    <div class="text-xs uppercase tracking-wider text-gray-500">Current plan</div>
                    <div class="text-2xl font-bold mt-1">{{ s.plan?.name || s.paymentStatus }}</div>
                    @if (s.plan) {
                        <div class="text-sm text-gray-600 mt-1">BDT {{ s.plan.monthlyFeeBDT | number:'1.0-2' }} / month · up to {{ s.plan.maxOutlets }} outlets · {{ s.plan.maxUsers }} users</div>
                    }
                </div>
                <div class="text-right">
                    <div class="text-xs uppercase tracking-wider text-gray-500">Valid until</div>
                    ...
                    <div class="text-xs mt-1" [class.text-rose-600]="s.daysUntilExpiry < 0" [class.text-amber-600]="s.daysUntilExpiry >= 0 && s.daysUntilExpiry <= 7">
                        {{ s.daysUntilExpiry < 0 ? 'Expired ' + (-s.daysUntilExpiry) + ' day(s) ago' : s.daysUntilExpiry + ' day(s) remaining' }}
                    </div>
                </div>
            </div>
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5 pt-4 border-t border-gray-200 dark:border-gray-700 text-sm">
                <div>
                    ...
                    <div class="font-medium mt-1" [class.text-emerald-600]="s.isSystemActive" [class.text-rose-600]="!s.isSystemActive">{{ s.isSystemActive ? s.paymentStatus : 'Suspended' }}</div>
                </div>
                <div>
                    <div class="text-xs uppercase tracking-wider text-gray-500">Last payment</div>
                    <div class="font-medium mt-1">{{ s.lastPaymentDate ? (s.lastPaymentDate | date:'mediumDate') : '—' }}</div>
                </div>
                <div>
                    <div class="text-xs uppercase tracking-wider text-gray-500">Next renewal</div>
                    <div class="font-medium mt-1">{{ s.nextBillingDate ? (s.nextBillingDate | date:'mediumDate') : '—' }}</div>
                </div>
                <div>
                    <div class="text-xs uppercase tracking-wider text-gray-500">Tenant</div>
                    ...
                    <p class="text-sm"
                       [ngClass]="s.severity === 'urgent' ? 'text-rose-800 dark:text-rose-200' : 'text-amber-800 dark:text-amber-200'">
                        @if (s.severity === 'urgent') {
                            ...
                            Your subscription expires in {{ s.daysUntilExpiry }} day(s). Contact your platform admin to renew.
                        }
                    </p>
                </div>
            }
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow border border-gray-200 dark:border-gray-700">
            <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between flex-wrap gap-3">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-xs text-gray-500">{{ payments().length }} record(s)</span>
                </div>
            ...
            @if (loadingPayments()) {
                <div class="p-5 text-gray-500 text-sm">Loading payments…</div>
            } @else if (payments().length === 0) {
                <div class="p-6 text-center text-gray-500 text-sm">No payments in this date range.</div>
            } @else {
                <table mat-table [dataSource]="payments()" class="w-full">
                    <ng-container matColumnDef="paidOn">
                        <th mat-header-cell *matHeaderCellDef class="pl-5"><span class="text-xs font-medium text-gray-500 uppercase">Paid on</span></th>
                        <td mat-cell *matCellDef="let p" class="pl-5">{{ p.paidOn | date:'mediumDate' }}</td>
                    </ng-container>
                    <ng-container matColumnDef="period">
                        <th mat-header-cell *matHeaderCellDef><span class="text-xs font-medium text-gray-500 uppercase">Period</span></th>
                        <td mat-cell *matCellDef="let p">{{ p.periodStart | date:'shortDate' }} → {{ p.periodEnd | date:'shortDate' }}</td>
                    </ng-container>
                    <ng-container matColumnDef="amount">
                        <th mat-header-cell *matHeaderCellDef class="!text-right"><span class="text-xs font-medium text-gray-500 uppercase">Amount</span></th>
                        <td mat-cell *matCellDef="let p" class="!text-right font-semibold">BDT {{ p.amount | number:'1.0-2' }}</td>
                    </ng-container>
                    <ng-container matColumnDef="method">
                        <th mat-header-cell *matHeaderCellDef><span class="text-xs font-medium text-gray-500 uppercase">Method</span></th>
                        <td mat-cell *matCellDef="let p">{{ p.method }}</td>
                    </ng-container>
                    <ng-container matColumnDef="reference">
                        <th mat-header-cell *matHeaderCellDef class="pr-5"><span class="text-xs font-medium text-gray-500 uppercase">Reference</span></th>
