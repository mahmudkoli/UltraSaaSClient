...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, RouterModule, MatButtonModule, MatCheckboxModule, MatDialogModule, MatFormFieldModule, MatIconModule, MatInputModule, MatPaginatorModule, MatSelectModule, MatSortModule, MatTableModule, MatTooltipModule, TranslocoModule],
    template: `
    ...
    <div class="relative z-10 flex flex-col flex-auto min-w-0">
        <div class="flex flex-col sm:flex-row space-y-16 sm:space-y-0 flex-0 sm:items-center sm:justify-between p-2 sm:py-3 sm:px-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h2 class="text-3xl font-bold tracking-tight leading-7 sm:leading-10 truncate bg-gradient-to-r from-gray-900 to-gray-600 dark:from-white dark:to-gray-300 bg-clip-text text-transparent">{{ 'CATALOG.PRODUCTS.LIST.TITLE' | transloco }}</h2>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ 'CATALOG.PRODUCTS.LIST.SUBTITLE' | transloco }}</p>
                </div>
            </div>
            <div class="flex flex-col w-full sm:w-auto sm:flex-row space-y-16 sm:space-y-0 flex-1 sm:flex-none sm:items-center sm:justify-end gap-4">
                <mat-form-field class="w-full sm:w-auto sm:min-w-72" appearance="outline" subscriptSizing="dynamic">
                    <mat-label>{{ 'CATALOG.PRODUCTS.LIST.SEARCH_LABEL' | transloco }}</mat-label>
                    <input matInput [(ngModel)]="search" (ngModelChange)="searchChanged.next($event)" [placeholder]="'CATALOG.PRODUCTS.LIST.SEARCH_PLACEHOLDER' | transloco">
                    <mat-icon matSuffix class="text-gray-400">search</mat-icon>
                </mat-form-field>
                <mat-form-field class="w-full sm:w-auto sm:min-w-48" appearance="outline" subscriptSizing="dynamic">
                    <mat-label>{{ 'CATALOG.PRODUCTS.LIST.CATEGORY_LABEL' | transloco }}</mat-label>
                    <mat-select [(ngModel)]="categoryFilter" (ngModelChange)="resetAndLoad()">
                        <mat-option value="">{{ 'CATALOG.PRODUCTS.LIST.CATEGORY_ALL' | transloco }}</mat-option>
                ...
                @if (selectedCount() > 0) {
                    <button mat-stroked-button class="!h-12 !px-4 !text-teal-700 !border-teal-300 !bg-teal-50 dark:!bg-teal-900/20"
                            ...
                            [matTooltip]="'CATALOG.PRODUCTS.LIST.PRINT_LABELS_TOOLTIP' | transloco">
                        <mat-icon class="icon-size-5 mr-1">qr_code_2</mat-icon>
                        <span>{{ 'CATALOG.PRODUCTS.LIST.PRINT_LABELS_BUTTON' | transloco:{ count: selectedCount() } }}</span>
                    </button>
                }
                <button mat-stroked-button class="!h-12 !px-4" (click)="openImport()" [matTooltip]="'CATALOG.PRODUCTS.LIST.IMPORT_TOOLTIP' | transloco"><mat-icon class="icon-size-5 mr-1">cloud_upload</mat-icon><span>{{ 'CATALOG.PRODUCTS.LIST.IMPORT_BUTTON' | transloco }}</span></button>
                <button mat-fab color="primary" routerLink="../products/create" [matTooltip]="'CATALOG.PRODUCTS.LIST.ADD_TOOLTIP' | transloco"><mat-icon>add</mat-icon></button>
            </div>
        </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div class="relative overflow-x-auto">
                    <table mat-table matSort [dataSource]="rows()" (matSortChange)="onSort($event)" class="w-full">
                        <ng-container matColumnDef="select">
                            <th mat-header-cell *matHeaderCellDef class="pl-4 sm:pl-6 !w-10">
                                <mat-checkbox
                                    ...
                                    [matTooltip]="'CATALOG.PRODUCTS.LIST.SELECT_ALL_TOOLTIP' | transloco"></mat-checkbox>
                            </th>
                        ...
                        <ng-container matColumnDef="sku"><th mat-header-cell *matHeaderCellDef mat-sort-header><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'CATALOG.PRODUCTS.LIST.COL_SKU' | transloco }}</span></th>
                            <td mat-cell *matCellDef="let r" class="font-mono text-xs">{{ r.sku }}</td></ng-container>
                        <ng-container matColumnDef="name"><th mat-header-cell *matHeaderCellDef mat-sort-header><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'CATALOG.PRODUCTS.LIST.COL_NAME' | transloco }}</span></th>
                        ...
                        <ng-container matColumnDef="costPrice"><th mat-header-cell *matHeaderCellDef mat-sort-header class="!text-right"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'CATALOG.PRODUCTS.LIST.COL_COST' | transloco }}</span></th>
                            <td mat-cell *matCellDef="let r" class="!text-right">{{ r.costPrice | number:'1.2-2' }}</td></ng-container>
                        <ng-container matColumnDef="sellingPrice"><th mat-header-cell *matHeaderCellDef mat-sort-header class="!text-right"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'CATALOG.PRODUCTS.LIST.COL_PRICE' | transloco }}</span></th>
                        ...
                        <ng-container matColumnDef="tax"><th mat-header-cell *matHeaderCellDef class="!text-right"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'CATALOG.PRODUCTS.LIST.COL_TAX' | transloco }}</span></th>
                            <td mat-cell *matCellDef="let r" class="!text-right">{{ r.taxRate | number:'1.0-2' }}</td></ng-container>
                        <ng-container matColumnDef="reorder"><th mat-header-cell *matHeaderCellDef class="!text-right"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'CATALOG.PRODUCTS.LIST.COL_REORDER' | transloco }}</span></th>
                            <td mat-cell *matCellDef="let r" class="!text-right">{{ r.reorderLevel | number:'1.0-2' }}</td></ng-container>
                        <ng-container matColumnDef="active"><th mat-header-cell *matHeaderCellDef><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'CATALOG.PRODUCTS.LIST.COL_STATUS' | transloco }}</span></th>
                            <td mat-cell *matCellDef="let r">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium" [ngClass]="r.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'">
                                    <mat-icon class="icon-size-4 mr-1">{{ r.isActive ? 'check_circle' : 'cancel' }}</mat-icon>{{ (r.isActive ? 'COMMON.ACTIVE' : 'COMMON.INACTIVE') | transloco }}
                                </span>
                            </td></ng-container>
                        <ng-container matColumnDef="actions"><th mat-header-cell *matHeaderCellDef class="pr-4 sm:pr-6 !text-right"><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ 'CATALOG.PRODUCTS.LIST.COL_ACTIONS' | transloco }}</span></th>
                            <td mat-cell *matCellDef="let r" class="pr-4 sm:pr-6">
                                <div class="flex items-center justify-end space-x-2">
                                    <button mat-icon-button class="text-blue-600" [routerLink]="['../products', r.id]" [matTooltip]="'CATALOG.PRODUCTS.LIST.ACTION_EDIT' | transloco"><mat-icon class="icon-size-5">edit</mat-icon></button>
                                    <button mat-icon-button class="text-teal-600" (click)="printLabels(r)" [matTooltip]="'CATALOG.PRODUCTS.LIST.ACTION_PRINT_LABELS' | transloco"><mat-icon class="icon-size-5">qr_code_2</mat-icon></button>
                                    <button mat-icon-button class="text-amber-600" (click)="manageOutletPrices(r)" [matTooltip]="'CATALOG.PRODUCTS.LIST.ACTION_OUTLET_PRICING' | transloco"><mat-icon class="icon-size-5">price_change</mat-icon></button>
                                    @if (showElectronics()) {
                                        <button mat-icon-button class="text-sky-600" (click)="manageElectronics(r)" [matTooltip]="'CATALOG.PRODUCTS.LIST.ACTION_ELECTRONICS' | transloco"><mat-icon class="icon-size-5">memory</mat-icon></button>
                                    }
                                    @if (showPharmacy()) {
                                        <button mat-icon-button class="text-emerald-600" (click)="managePharmacy(r)" [matTooltip]="'CATALOG.PRODUCTS.LIST.ACTION_PHARMACY' | transloco"><mat-icon class="icon-size-5">medication</mat-icon></button>
                                    }
                                    <button mat-icon-button class="text-red-600" (click)="remove(r)" [matTooltip]="'CATALOG.PRODUCTS.LIST.ACTION_DELETE' | transloco"><mat-icon class="icon-size-5">delete</mat-icon></button>
                                </div>
                            </td></ng-container>
                ...
                <div *ngIf="!loading() && rows().length === 0" class="flex flex-col items-center justify-center p-12">
                    ...
                    <h3 class="text-xl font-semibold text-gray-900 dark:text-white mb-3">{{ 'CATALOG.PRODUCTS.LIST.EMPTY_TITLE' | transloco }}</h3>
                    <button *ngIf="!search && !categoryFilter" mat-flat-button color="primary" routerLink="../products/create"><mat-icon class="icon-size-5 mr-2">add_circle</mat-icon><span>{{ 'CATALOG.PRODUCTS.LIST.ADD_BUTTON' | transloco }}</span></button>
                </div>
            </div>
        </div>
    </div>
...
export class ProductListComponent implements OnInit {
    ...
    remove(r: ProductDto): void {
        this._confirm.open({
            title: this._transloco.translate('CATALOG.PRODUCTS.DELETE_TITLE'),
            message: this._transloco.translate('CATALOG.PRODUCTS.DELETE_CONFIRM', { name: r.name }),
            icon: { show: true, name: 'heroicons_outline:exclamation-triangle', color: 'warn' },
            actions: { confirm: { label: this._transloco.translate('COMMON.DELETE'), color: 'warn' }, cancel: { label: this._transloco.translate('COMMON.CANCEL') } },
    ...
    openImport(): void {
        const config: ImportDialogConfig = {
            title: this._transloco.translate('CATALOG.PRODUCTS.LIST.IMPORT_DIALOG_TITLE'),
            subtitle: this._transloco.translate('CATALOG.PRODUCTS.LIST.IMPORT_DIALOG_SUBTITLE'),
