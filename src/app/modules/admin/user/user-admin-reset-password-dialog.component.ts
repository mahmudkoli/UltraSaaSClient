...
<div class="p-1">
    <div class="flex items-center space-x-3 px-4 pt-4 pb-2">
        ...
        <div>
            <h2 class="text-xl font-semibold text-gray-900 dark:text-white">{{ 'ADMIN.USER.RESET_DIALOG.TITLE' | transloco }}</h2>
    ...
    <mat-dialog-content class="!p-4">
        <div class="rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 mb-4 text-xs text-amber-900 dark:border-amber-600 dark:bg-amber-900/30 dark:text-amber-200">
            {{ 'ADMIN.USER.RESET_DIALOG.WARNING' | transloco }}
        </div>

        <mat-form-field appearance="outline" class="w-full">
            <mat-label>{{ 'ADMIN.USER.RESET_DIALOG.NEW_PASSWORD_LABEL' | transloco }}</mat-label>
            <input matInput
                   [type]="showPassword() ? 'text' : 'password'"
                   [(ngModel)]="password"
                   [placeholder]="'ADMIN.USER.RESET_DIALOG.NEW_PASSWORD_PLACEHOLDER' | transloco"
                   autocomplete="new-password"
                   (input)="error.set('')">
            <button matSuffix mat-icon-button type="button"
                    (click)="showPassword.set(!showPassword())"
                    [matTooltip]="(showPassword() ? 'ADMIN.USER.RESET_DIALOG.HIDE_TOOLTIP' : 'ADMIN.USER.RESET_DIALOG.SHOW_TOOLTIP') | transloco">
        ...
        <button mat-stroked-button class="!mt-1" (click)="generate()" [disabled]="saving()">
            <mat-icon class="icon-size-5 mr-1">casino</mat-icon>{{ 'ADMIN.USER.RESET_DIALOG.GENERATE' | transloco }}
        </button>
    ...
    <mat-dialog-actions align="end" class="!px-4 !pb-4">
        ...
        <button mat-flat-button color="warn"
            ...
            <mat-icon class="icon-size-5 mr-2">lock_reset</mat-icon>{{ (saving() ? 'ADMIN.USER.RESET_DIALOG.RESETTING' : 'ADMIN.USER.RESET_DIALOG.RESET_BUTTON') | transloco }}
        </button>
    </mat-dialog-actions>
</div>
    `,
...
export class UserAdminResetPasswordDialogComponent {
    ...
    save(): void {
        if (this.password.length < 8) {
            this.error.set(this._transloco.translate('ADMIN.USER.RESET_DIALOG.MIN_LENGTH_ERROR'));
            return;
        }
        ...
        this.userService.adminResetPassword(this.data.userId, this.password).subscribe({
            ...
            error: (err) => {
                ...
                this.error.set(err?.error?.message || err?.message || this._transloco.translate('ADMIN.USER.RESET_DIALOG.RESET_FAILED'));
            },
        });
    }
