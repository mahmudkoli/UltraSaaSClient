    ...
    <div class="relative z-10 flex flex-col flex-auto min-w-0">
        <div class="flex flex-col sm:flex-row space-y-16 sm:space-y-0 flex-0 sm:items-center sm:justify-between p-2 sm:py-3 sm:px-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h2 class="text-3xl font-bold tracking-tight leading-7 sm:leading-10 truncate bg-gradient-to-r from-gray-900 to-gray-600 dark:from-white dark:to-gray-300 bg-clip-text text-transparent">New Stock Adjustment</h2>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Set the new quantity directly. The adjustment Δ is computed from the current stock.</p>
                </div>
            </div>
            <button mat-stroked-button class="h-12 px-6 rounded-lg" routerLink="/stock-adjustments"><mat-icon class="icon-size-5 mr-2">arrow_back</mat-icon><span>Cancel</span></button>
        </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden p-6 lg:col-span-2">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <mat-form-field appearance="outline" class="w-full">
                            ...
                            <mat-select [(ngModel)]="reason">
                                ...
                                <mat-option value="Correction">Correction (data fix)</mat-option>
                    ...
                    <div class="flex items-center justify-end gap-2 pt-4 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <button mat-flat-button color="primary" type="button" class="h-12 px-6 rounded-lg shadow-lg"
                                ...
                                (click)="save()">
                            <mat-icon class="icon-size-5 mr-2">save</mat-icon><span>{{ saving ? 'Saving…' : 'Save Adjustment' }}</span>
                        </button>
                    </div>
...
export class StockAdjustmentFormComponent implements OnInit {
    ...
    disabledReason(): string {
        ...
        if (this.newQuantity == null) return 'Enter the new quantity.';
        if (this.newQuantity < 0) return 'New quantity cannot be negative.';
        return '';
    }
