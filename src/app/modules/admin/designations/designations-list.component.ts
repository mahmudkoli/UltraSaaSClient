...
export class DesignationsListComponent implements OnInit, OnDestroy {
    ...
    displayedColumns = ['name', 'code', 'level', 'status', 'actions'];
    Math = Math;
    ...
    load(): void {
        ...
        this._service.search({
            ...
            error: () => {
                this._notificationService.error('Failed to load designations');
                this.isLoading = false;
                this._changeDetectorRef.markForCheck();
            },
        });
        ...
        confirmation.afterClosed().subscribe((result) => {
            if (result === 'confirmed') {
                this._service.delete(item.id).pipe(takeUntil(this._unsubscribeAll)).subscribe({
                    next: () => { this._notificationService.success('Designation deleted'); this.load(); },
                    error: () => this._notificationService.error('Failed to delete designation'),
                });
            }
        });
    }
