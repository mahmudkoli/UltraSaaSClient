                        ...
                        <ng-container matColumnDef="expiry"><th mat-header-cell *matHeaderCellDef><span class="text-xs font-medium text-gray-500 uppercase tracking-wider">Expiry</span></th>
                            <td mat-cell *matCellDef="let r"
                                ...
                                <div class="flex flex-col">
                                    <span>{{ r.expiryDate ? (r.expiryDate | date:'shortDate') : '—' }}</span>
...
export class BatchesListComponent implements OnInit {
    private readonly api = inject(BatchesService);
    ...
    statusIcon(s: BatchDto['status']): string {
        switch (s) {
            case 'Active': return 'check_circle';
            case 'Exhausted': return 'inventory';
            case 'Expired': return 'event_busy';
            case 'Recalled': return 'report';
            default: return 'help';
        }
    }
    ...
    badgeClass(s: BatchDto['status']): Record<string, boolean> {
        return {
            ...
            'bg-gray-100 text-gray-800 dark:bg-gray-900 dark:text-gray-200': s === 'Exhausted',
            'bg-orange-100 text-orange-800 dark:bg-orange-900 dark:text-orange-200': s === 'Expired',
            'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200': s === 'Recalled',
        };
    }

    expiryClass(r: BatchDto): Record<string, boolean> {
        if (!r.expiryDate) return {};
        return {
            'text-rose-700 dark:text-rose-400': r.daysToExpiry <= 0,
            'text-amber-700 dark:text-amber-400': r.daysToExpiry > 0 && r.daysToExpiry <= 30,
        };
    }
