...
import { AnnouncementAudienceKind, NotificationAudience, NotificationSeverity } from 'app/core/billing/billing.types';

/**
 * Platform-admin announcement broadcast. Single page with the create form
 * inline (no separate list view yet — the backend doesn't return historical
 * announcements as a single entity since we fan-out at create time, and
 * showing per-tenant fanned-out rows would just be noise. Add a real
 * "AnnouncementHistory" entity when first asked for it.)
 */
@Component({
    ...
    template: `
<div class="flex flex-col flex-auto min-w-0 p-4 sm:p-6 max-w-3xl">
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-bold">Broadcast announcement</h1>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow border border-gray-200 dark:border-gray-700 p-5">
        ...
        @if (successMsg()) {
            <p class="text-sm text-emerald-700 mt-2">{{ successMsg() }}</p>
        }

        <div class="flex justify-end gap-2 mt-4">
            <button mat-button (click)="reset()" [disabled]="busy()">Reset</button>
...
export class AnnouncementsComponent {
    ...
    audienceTarget = '';
    visibleToAllUsers = false; // false → AdminOnly (default per Phase 2.48 authz model)
    linkUrl = '';
    ...
    reset(): void {
        this.title = ''; this.body = ''; this.severity = 'Info';
        this.audienceKind = 'All'; this.audienceTarget = '';
        this.visibleToAllUsers = false; this.linkUrl = '';
    ...
    submit(): void {
        ...
        this.api.create({
            ...
            next: r => {
                this.busy.set(false);
                this.successMsg.set(`Delivered to ${r.deliveredTo} tenant(s).`);
